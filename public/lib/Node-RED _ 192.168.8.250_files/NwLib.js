...
(function (context, undefined) {
    ...
    var Nw = context.Class({
        ...
        //invoke method Asynchonus with limit per timeout
        InvokerCallback: function (invokeFunc) {
            var invokeFunc;

            var waitingTark = { argObj: null, cb: null };
            var currentTark = { argObj: null, cb: null };
            if (invokeFunc) {
                invokeFunc = invokeFunc;
            }


            var callInvokeFunc = function (tark, cb) {
                invokeFunc(tark.argObj, function (result) {
                    tark.argObj = null;
                    if (tark.cb) tark.cb(result);
                    cb();
                });
            };


            var callWaitingTark = function () {
                currentTark = waitingTark;
                waitingTark.argObj = null;

                if (currentTark.argObj) {
                    callInvokeFunc(currentTark, function () {
                        callWaitingTark();
                    });
                }
            };
            ...
            this.invoke = function (argObj, cb) {

                if (currentTark.argObj == null) {
                    currentTark.argObj = argObj;
                    currentTark.cb = cb;
